...
import { Et as parsePartialJson, F as callCompletionApi, cn as resolve, en as asSchema, in as isAbortError, ln as safeValidateTypes, mt as isDeepEqualData, on as normalizeHeaders, t as AbstractChat } from "./dist-Docys6LQ.js";
...
//#endregion
//#region node_modules/swr/dist/_internal/config-context-12s-CCVTDPOP.mjs
var SWRGlobalState = /* @__PURE__ */ new WeakMap();
var noop$1 = () => {};
var UNDEFINED$1 = noop$1();
var OBJECT = Object;
var isUndefined = (v) => v === UNDEFINED$1;
...
var initCache = (provider, options) => {
	if (!SWRGlobalState.has(provider)) {
		...
		const mutate = internalMutate.bind(UNDEFINED$1, provider);
		let unmount = noop$1;
		...
		const initProvider = () => {
			if (!SWRGlobalState.has(provider)) {
				...
				if (!IS_SERVER) {
					const releaseFocus = opts.initFocus(setTimeout.bind(UNDEFINED$1, revalidateAllKeys.bind(UNDEFINED$1, EVENT_REVALIDATORS, 0)));
					const releaseReconnect = opts.initReconnect(setTimeout.bind(UNDEFINED$1, revalidateAllKeys.bind(UNDEFINED$1, EVENT_REVALIDATORS, 1)));
					unmount = () => {
...
var SWRConfig$1 = (props) => {
	...
	const cacheContextRef = (0, import_react.useRef)(UNDEFINED$1);
...
var resolvedUndef = Promise.resolve(UNDEFINED$1);
var sub = () => noop$1;
/**
...
*/ var useSWRHandler = (_key, fetcher, config) => {
	...
	const fallback = isUndefined(fallbackData) ? isUndefined(config.fallback) ? UNDEFINED$1 : config.fallback[key] : fallbackData;
...
function useCompletion({ api = "/api/completion", id, initialCompletion = "", initialInput = "", credentials, headers, body, streamProtocol = "data", fetch: fetch2, onFinish, onError, experimental_throttle: throttleWaitMs } = {}) {
	...
	const triggerRequest = (0, import_react.useCallback)(async (prompt, options) => callCompletionApi({
		api,
		prompt,
	...
	const handleSubmit = (0, import_react.useCallback)((event) => {
		var _a;
		(_a = event == null ? void 0 : event.preventDefault) == null || _a.call(event);
...
function useObject({ api, id, schema, initialValue, fetch: fetch2, onError, onFinish, headers, credentials }) {
	const hookId = (0, import_react.useId)();
	const { data, mutate } = useSWR([api, id != null ? id : hookId], null, { fallbackData: initialValue });
	const [error, setError] = (0, import_react.useState)(void 0);
	...
	const stop = (0, import_react.useCallback)(() => {
		var _a;
		try {
			(_a = abortControllerRef.current) == null || _a.abort();
	...
	const submit = async (input) => {
		...
		try {
			...
			const response = await (fetch2 != null ? fetch2 : getOriginalFetch())(api, {
				method: "POST",
				headers: {
			...
			if (!response.ok) throw new Error((_a = await response.text()) != null ? _a : "Failed to fetch the response.");
